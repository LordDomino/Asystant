package main;

public class Datatypes {
    public static void main(String[] args) {
        // Form Reference Section A
        String a_student_fullname;
        String a_student_firstname;
        String a_student_middlename;
        String a_student_middlename_initial;
        String a_student_lastname;
        String a_level_section;
        int a_gradelevel;
        String a_section;
        String a_address;
        String a_address_city;
        String a_address_brgy;
        String a_address_st;
        int a_address_bl;
        int a_address_lt;
        String a_birthdate;
        String a_birthday;
        int a_birthday_val;
        String a_birthmonth;
        int a_birthmonth_val;
        int a_birthyear;
        float a_age;
        float a_height_ft;
        float a_height_m;
        float a_weight_kg;
        float a_weight_lbs;
        String a_mother_fullname;
        String a_mother_firstname;
        String a_mother_middlename;
        String a_mother_middlename_initial;
        String a_mother_lastname;
        String a_mother_occupation;
        int a_mother_contactno;
        String a_mother_email;
        String a_father_fullname;
        String a_father_firstname;
        String a_father_middlename;
        String a_father_middlename_initial;
        String a_father_lastname;
        String a_father_occupation;
        int a_father_contactno;

        // Form Reference Section B
        String a_father_email;
        String b_contactperson_fullname;
        String b_contactperson_firstname;
        String b_contactperson_middlename;
        String b_contactperson_middlename_initial;
        String b_contactperson_lastname;
        String b_contactperson_relationship; // does not denote if "single" or "married"!
        int b_contactperson_contactno;

        // Form Reference Section C
        boolean c_question1;
        String c_question1_details;
        boolean c_checkbox_food;
        boolean c_checkbox_insectbite;
        boolean c_checkbox_medication;
        boolean c_checkbox_others;
        String c_checkbox_food_details;
        String c_checkbox_insectbite_details;
        String c_checkbox_medication_details;
        String c_checkbox_others_details;

        // Form Reference Section D
        boolean d_question1;
        boolean d_checkbox_vitamins;
        boolean d_checkbox_medicationforallergy;
        boolean d_checkbox_medicationforasthma;
        boolean d_checkbox_medicationforpresentpastillnesssickness;
        boolean d_checkbox_others;
        String d_checkbox_vitamins_details;
        String d_checkbox_medicationforallergy_details;
        String d_checkbox_medicationforasthma_details;
        String d_checkbox_medicationforpresentpastillnesssickness_details;
        String d_checkbox_others_details;

        // Form Reference Section E
        boolean e_is_asthma;
        boolean e_is_g6pd;
        boolean e_is_stomachproblems;
        boolean e_is_diabetic;
        boolean e_is_lungproblem;
        boolean e_is_heartdisorder;
        boolean e_is_uti;
        boolean e_is_fainting_or_dizzy;
        boolean e_is_eyeproblem;
        boolean e_is_fracture;
        boolean e_is_allergicrhinitis;
        boolean e_is_eczema;
        boolean e_is_febrileseizures;
        boolean e_is_dengue;
        boolean e_is_pneumonia;
        boolean e_is_hfmd;
        boolean e_is_chickenpox;
        boolean e_is_measles;
        boolean e_is_wearingeyeglasses;
        boolean e_is_hearingdifficult;
        boolean e_is_otheractivemedicalconditions;
        String e_asthma_details;
        String e_g6pd_details;
        String e_stomachproblems_details;
        String e_diabetic_details;
        String e_lungproblem_details;
        String e_heartdisorder_details;
        String e_uti_details;
        String e_fainting_or_dizzy_details;
        String e_eyeproblem_details;
        String e_fracture_details;
        String e_allergicrhinitis_details;
        String e_eczema_details;
        String e_febrileseizures_details;
        String e_dengue_details;
        String e_pneumonia_details;
        String e_hfmd_details;
        String e_chickenpox_details;
        String e_measles_details;
        int e_righteye_details;
        int e_lefteye_details;
        String e_hearingdifficult_details;
        String e_otheractivemedicalconditions_details;
        String e_eyegrade;

        // Form Reference Section F
        boolean f_is_basicimmunization;
        boolean f_is_hepatitisB;
        boolean f_is_dtap;
        boolean f_is_polio;
        boolean f_is_rotavirus;
        boolean f_is_pcv;
        boolean f_is_chickenpox_varicella;
        boolean f_is_mmr;
        boolean f_is_hepatitisA;
        boolean f_is_flu;
        boolean f_is_others;
        String f_others_details;
    
        // Form Reference Section G

        boolean              g_hasthechildcontractedcovid_19virus;
        String               g_if_true_provideinformatioString;
        String               g_if_true_dates_contractedvirus;
        int | Date           g_if_true_dates_contractedvirus;
        boolean              g_if_true_undergocovid19confirmatorytest;
        boolean | String     g_if_yes_checkcovid19confirmatorytest;
        String               g_if_yes_checkcovid19confirmatorytest_others;
        boolean              g_is_Asymtomatic;
        boolean              g_is_quarantinedinfacilityorhomeisolation;
        String               g_if_yes_statelocationanddateofisolationquarantine;
        boolean              g_totaldaysofisolation;
        boolean              g_if_yourchildhasalreadygivencovid19vaccine;
    }
}